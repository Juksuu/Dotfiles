...
local plug_map = {
    ...
    ["i|<CR>"] = map_cmd(
        [[compe#confirm({ 'keys': "\<Plug>delimitMateCR", 'mode': '' })]]):with_noremap()
        :with_expr():with_nowait(),
    ...
    ["n|<leader>cs"] = map_cr('Lspsaga signature_help'):with_noremap()
        :with_silent(),
    ["n|<Leader>dl"] = map_cr('Lspsaga show_line_diagnostics'):with_noremap()
        :with_silent(),
    ["n|<leader>dn"] = map_cr('Lspsaga diagnostic_jump_next'):with_noremap()
        :with_silent(),
    ["n|<leader>dp"] = map_cr('Lspsaga diagnostic_jump_prev'):with_noremap()
        :with_silent(),
...
local map_tele = function(key, f, options, buffer)
    ...
    local rhs = string.format(
                    "<cmd>lua R('modules.utils.telescope')['%s'](%s)<CR>", f,
