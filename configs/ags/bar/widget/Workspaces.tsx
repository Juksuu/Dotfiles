    ...
    return (
      <DrawingArea
        ...
        setup={(self) => {
          ...
          self.hook(bind(hyprland, "focused_workspace"), (self) => {
            onFocusedWorkspace(self, count);
          });
        ...
        onDraw={(self: DrawingArea, cr: Cairo.Context) => {
          const workspace = hyprland.get_focused_workspace();
          const offset =
            Math.floor((workspace.id - 1) / count) * workspacesShown;
          ...
          const wsfg = wsStyleContext.get_property(
            "color",
            Gtk.StateFlags.NORMAL,
          ) as Gdk.RGBA;
          const wsDiameter = wsStyleContext.get_property(
            "min-width",
            Gtk.StateFlags.NORMAL,
          ) as number;
          const wsFontSize = wsStyleContext.get_property(
            "font-size",
            Gtk.StateFlags.NORMAL,
          ) as any;
          const wsFontFamily = wsStyleContext.get_property(
            "font-family",
            Gtk.StateFlags.NORMAL,
          ) as any;
          const wsFontWeight = wsStyleContext.get_property(
            "font-weight",
            Gtk.StateFlags.NORMAL,
          ) as any;

          const occupiedWsfg = occupiedWsStyleContext.get_property(
            "color",
            Gtk.StateFlags.NORMAL,
          ) as any;
          const occupiedWsbg = occupiedWsStyleContext.get_property(
            "background-color",
            Gtk.StateFlags.NORMAL,
          ) as any;

          const activeWsfg = activeWsStyleContext.get_property(
            "color",
            Gtk.StateFlags.NORMAL,
          ) as any;
          const activeWsbg = activeWsStyleContext.get_property(
            "background-color",
            Gtk.StateFlags.NORMAL,
          ) as any;
          ...
          for (let i = 1; i <= count; i++) {
            if (workspaceMask & (1 << i)) {
              ...
              const wsCenter = [-wsRadius + wsDiameter * i, height / 2];
          ...
          for (let i = 1; i <= count; i++) {
            const inactivecolors =
              workspaceMask & (1 << i) ? occupiedWsfg : wsfg;
